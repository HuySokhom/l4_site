...
class Application extends Container implements HttpKernelInterface, TerminableInterface, ResponsePreparerInterface
{
    ...
    {
        return 'C:\\wamp\\www\\l4_site\\vendor\\laravel\\framework\\src\\Illuminate\\Foundation' . '/start.php';
    }
        ...
        $this->app['events']->listen('router.matched', function () {
            $this->app->resolvingAny(function ($resolved, $app) {
                if ($resolved instanceof FormRequest) {
                    ...
                    $resolved->setContainer($app)->setRoute($app['Illuminate\\Routing\\Route'])->setRedirector($app['Illuminate\\Routing\\Redirector'])->validate($app['Illuminate\\Validation\\Factory']);
                }
            });
        });
    }
